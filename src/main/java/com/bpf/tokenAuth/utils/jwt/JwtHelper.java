package com.bpf.tokenAuth.utils.jwt;
...
/**
 * @Author: Helon
 * @Description: JWT工具类 参考官网：https://jwt.io/
 *               实现机制参考博客：https://www.cnblogs.com/tyrion1990/p/8134384.html
 *               JWT的数据结构为：A.B.C三部分数据，由字符点"."分割成三部分数据 A-header头信息 B-payload 有效负荷
 *               一般包括：已注册信息（registered claims），公开数据(public claims)，私有数据(private
 *               claims) C-signature 签名信息 是将header和payload进行加密生成的
...
public class JwtHelper {
	...
	/**
	 * @Author: Helon
	 * @Description: 生成JWT字符串 格式：A.B.C A-header头信息 B-payload 有效负荷 C-signature 签名信息
	...
	public static String generateJWT(String userId, String userName, String... identities) {
		...
		JwtBuilder builder = Jwts.builder().setHeader(headMap)
				/*
				 * Payload { "userId": "1234567890", "userName": "John Doe", }
				 */
				// 加密后的客户编号
	...
	/**
	 * @Author: Helon
	 * @Description: 校验JWT是否有效 返回json字符串的demo:
	 *               {"freshToken":"A.B.C","userName":"Judy","userId":"123",
	 *               "userAgent":"xxxx"} freshToken-刷新后的jwt userName-客户名称
	 *               userId-客户编号 userAgent-客户端浏览器信息
	 * @param jsonWebToken - JWT
