...
function getImgLoadEd(loadingBox, callback) {
    //存放 指定要加载的图片 的盒子
    let imgAll = document.getElementById(loadingBox);
    //指定要加载的图片 的数量
    let imgL = imgAll.children.length;
    //指定要加载的图片 起始 key
    let imgStart = 0;
    ...
    function IfLoadImg() {
        ...
        function loadImg(imgKey) {
            let curImg = imgAll.children[imgKey].children[0].children[0].children[0].src;
...
function waterFlow(parent, chirld) {
    let wparent = document.getElementById(parent);//获取父级div, 最外级容器
    let allArr = getAllChirld(wparent, chirld);//获取到所有的class为box的容器div
    let wscreenWidth = document.documentElement.clientWidth;//获取屏幕宽度
    let wchirldWidth = wparent.getElementsByTagName("*");//获取所有的标签
    let num = Math.floor(wscreenWidth / wchirldWidth[0].offsetWidth);//这是一个Math算法, 目的是将小数转变为整数,
    // 从而可以知道每行最多容纳几张图片
...
function getAllChirld(parent, classname) {
    //获取所有的标签
    let wchirld = parent.getElementsByTagName("*");
    //创建数组
    let chirldArr = [];
    //遍历wchirld, 将其中className等于classname(传进来的参数)相同的标签放入数组chirldArr中
    for (let i = 0; i < wchirld.length; i++) {
...
function getMinHeightOfCols(chirdArr, num) {
    ...
    for (let i = 0; i < chirdArr.length; i++) {
        ...
        if (i < num) {
            ...
            let minHeightOfindex = getminIndex(onlyOneColsArr, minHeightOfCols);
            //定义布局方式为绝对布局
